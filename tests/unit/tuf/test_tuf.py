...
class TestTUF:
    ...
    def test_post_targets(self, monkeypatch):
        payload = {
            "targets": [
                {
                    "path": "foo",
        ...
        # Test success
        result = tuf.post_targets(self.server, payload)
        ...
        with pytest.raises(tuf.RSTUFNoBootstrapError):
            tuf.post_targets(self.server, payload)
    ...
    def test_update_metadata(self, db_request, monkeypatch):
        ...
        monkeypatch.setattr(tuf, "post_targets", post)
